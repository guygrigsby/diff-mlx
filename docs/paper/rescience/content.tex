% Body of the ReScience C submission. Kept in sync with ../main.tex (the
% plain-article draft) by hand; the numbers cannot drift because both input
% the same generated/ files, which the Makefile copies in before building.
...
paper\supercite{ye2025differential} reports consistent wins at 3B
...
a spurious win. We control for this three ways. Every
...
MLX\supercite{mlx2023} with rotary position
embeddings\supercite{su2024roformer}, SwiGLU\supercite{shazeer2020glu},
RMSNorm\supercite{zhang2019rmsnorm} and tied embeddings, with standard
multi-head attention\supercite{vaswani2017attention} as the baseline.
The differential variant follows the paper's canonical form. Diff heads
pair adjacent attention heads with an interleaved split, $\lambda$
vectors are stored fp32 with the paper's depth schedule and a per-head
RMSNorm applies after the subtraction. The forward path uses the
linearity identity $(A_1 - \lambda A_2)V = A_1 V - \lambda A_2 V$, so
no $T \times T$ attention map is materialized.
...
convention bugs during development. One was an interleaved head-pair split the internal oracle missed because the oracle shared the bug. The other was a RoPE convention mismatch. Second, the custom kernels are validated against
...
Two scales, both on FineWeb-Edu\supercite{penedo2024fineweb} tokenized
with \texttt{cl100k\_base} at context length 2048. Stage 0 trains 30M
...
Separately, mid-run throughput dropped about ten percent while the chip
sat cool at 73$^\circ$C, with the battery slowly draining under a
reported charging state. The cause was power delivery. The machine was
charging through a Thunderbolt dock that also drove a monitor, and
switching to the direct first-party brick restored clocks and
throughput. The 14-inch machine negotiates at most 100 W regardless of
the adapter (its fast-charge spec is 96 W and it does not use the
higher EPR profile), so sustained training plus charging leaves little
headroom, and a dock delivering less than its rated upstream power
pushes the shortfall onto the battery and the GPU clocks. A low
temperature does not rule out throttling. Confirm the negotiated
adapter wattage before benchmarking, and state cooling and power
conditions with any published throughput number.
